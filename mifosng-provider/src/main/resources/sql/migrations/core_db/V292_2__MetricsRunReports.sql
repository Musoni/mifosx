...
FROM (
	...
	select ounder.id as officeId, ml.id, mst.id as staffId, (ml.interest_charged_derived-IFNULL(mlt.interest_paid,0) ) as IOUT
		FROM
			m_office mo
			JOIN m_office ounder ON ounder.hierarchy LIKE CONCAT(mo.hierarchy, '%')
			AND ounder.hierarchy like CONCAT('.', '%')
			...
			select ounder.id as officeId, ml.id, mst.id as staffId, (ml.interest_charged_derived-IFNULL(mlt.interest_paid,0) ) as POUT
			FROM
			m_office mo
			JOIN m_office ounder ON ounder.hierarchy LIKE CONCAT(mo.hierarchy, '%')
			AND ounder.hierarchy like CONCAT('.', '%')
...
FROM (
	...
	select ounder.id as officeId, ml.id, mst.id as staffId,
			IFNULL(total_writtenoff_derived,0) as writteoff
			...
			where  ( ml.writtenoffon_date <= LAST_DAY(NOW()) AND ml.writtenoffon_date  >= DATE_FORMAT(NOW(),'%Y-%m-01') ) AND ml.writtenoffon_date is not null
...
FROM (
	...
	select
			...
			IF(transaction_type_enum=8,IFNULL(lt.recovered_portion_derived,lt.overpayment_portion_derived),(ifnull(lt.principal_portion_derived,0) + ifnull(lt.interest_portion_derived,0) + ifnull(lt.fee_charges_portion_derived,0) + ifnull(lt.penalty_charges_portion_derived,0) + ifnull(lt.overpayment_portion_derived,0))) as pif
			FROM m_office mo
			JOIN m_office ounder ON ounder.hierarchy LIKE CONCAT(mo.hierarchy, '%')
			AND ounder.hierarchy like CONCAT('.', '%')
